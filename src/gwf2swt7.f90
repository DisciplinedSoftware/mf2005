      MODULE GWFSWTMODULE
        IMPLICIT NONE
        INTEGER,SAVE,POINTER:: ISWTCB
        INTEGER,SAVE,POINTER:: ISWTOC
        INTEGER,SAVE,POINTER:: NSYSTM
        INTEGER,SAVE,POINTER:: ITHK
        INTEGER,SAVE,POINTER:: IVOID
        INTEGER,SAVE,POINTER:: ISTPCS
        INTEGER,SAVE,POINTER:: ICRCC
        INTEGER,SAVE,POINTER:: IZCFL
        INTEGER,SAVE,POINTER:: IZCFM
        INTEGER,SAVE,POINTER:: IGLFL
        INTEGER,SAVE,POINTER:: IGLFM
        INTEGER,SAVE,POINTER:: IESTFL
        INTEGER,SAVE,POINTER:: IESTFM
        INTEGER,SAVE,POINTER:: IPCSFL
        INTEGER,SAVE,POINTER:: IPCSFM
        INTEGER,SAVE,POINTER:: ISTFL
        INTEGER,SAVE,POINTER:: ISTFM
...
!
      TYPE GWFSWTTYPE
        ...
        INTEGER,POINTER:: ISTFL
        INTEGER,POINTER:: ISTFM
...
! ------ARE CONSISTENT WITH STARTING EFFECTIVE STRESS VALUES.
...
!     ------------------------------------------------------------------
     ...
     &                 VOID,PCS,GL,ZC,EST,ESTOLD,SGM,SGS,ITHK,LNWT
      ...
      DOUBLE PRECISION PCTMP
      DOUBLE PRECISION ESTN1
      DOUBLE PRECISION ESTN
!     ------------------------------------------------------------------
      CALL SGWF2SWT7PNT(IGRID)
!
...
! ------SAVE VERTICAL DISPLACEMENT FOR ALL MODEL LAYERS.
     ...
     &              NROW,KKL,ISWOCU(4))
      ...
      ENDIF
!
! ------PRINT AND SAVE PRECONSOLIDATION STRESS
     ...
     &          NROW,KKL,ISWOCU(5))
      ...
      ENDIF
!
! ------PRINT AND SAVE CHANGE IN PRECONSOLIDATION STRESS
...
!     ------------------------------------------------------------------
      ...
      DIMENSION BOTM(NCOL,NROW,0:Nlay)
      DIMENSION ZC(NCOL,NROW,NLAY)
!
      DO K=1,NLAY
      DO IR=1,NROW
      DO IC=1,NCOL
...
!     ------------------------------------------------------------------
      ...
      DIMENSION SGM(NCOL,NROW)
      DIMENSION SGS(NCOL,NROW)
      DIMENSION IBOUND(NCOL,NROW,NLAY)
      DIMENSION HNEW(NCOL,NROW,NLAY)
      DIMENSION BOTM(NCOL,NROW,0:Nlay)
!
...
!
     ...
     &     'SUMMARY OF NEGATIVE EFFECTIVE STRESS LOCATIONS',&
     &     '     COUNT', '  ROW', '  COL', '  LAY',&
     &     '    BOTTOM', '       TOP', '      HEAD',&
     &     'GEO.STRESS', 'PRESS.HEAD', 'EFF.STRESS'
        ENDIF
        WRITE(IOUT,'(1X,I10,1X,3(I5,1X),6(F10.3,1x))')&
     &    INEGCNT, IR, IC, K, BOTM(IC,IR,K-1), BBOTM, HHNEW,&
     &    GL(IC,IR,K), HHNEW-BBOTM, EST(IC,IR,K)
      ENDIF
      ENDDO
      ENDDO
      ENDDO
