...
!
!     ******************************************************************
!     CHECK FOR STREAMBED BELOW CELL BOTTOM. RECORD REACHES FOR PRINTING
!     ******************************************************************
...
!         6 ASSIGNED TO SEGMENTS THAT DIVERT FLOW.
...
!dep revised pp1,pp2,et1,and et2, wrong placement of parenthesis.
...
!dep revised flwpet1, wrong placement of parenthesis.
...
!         IF REQUESTED (ISTCB2>0).
     ...
     &                            STRM(5, l), STRM(16, l), SNGL(grad)
           ...
           END IF
!
!42-----PRINT STREAMFLOWS AND RATES FOR EACH REACH TO MAIN LIST
...
!3------CALCULATE TABLES FOR LAKE STAGE AND CHANGE IN LAKE OUTFLOW.
          DO lk = 1, 200
            IF ( lk.EQ.1 )THEN
...
!         AVAILABLE STREAMFLOW.
      ELSE IF ( Iprior.EQ.-2 ) THEN
        ...
        Dvrsn = Upflw*prcnt
!
!4------IF IPRIOR IS -3 THEN FLOW DIVERTED ONLY WHEN STREAMFLOW
...
!8------CALCULATE WETTED PERIMETER.
            xx = ABS(xmid-XSEC(mark(ll), Istsg))
            yy = ABS(ffmax-ffmin)
            wetted = wetted + SQRT((xx**2)+(yy**2))
!9------BREAK AREA UP INTO TRAPEZOIDS.
            DO ii = 1, 50
              y1 = slope*xmid + b
...
!     OF TIME TO CALCULATE SPECIFIED INFLOW TO SEGMENTS.
