...
!--------------------------------------------------------------------
      MODULE GMGMODULE
        USE ISO_C_BINDING
        INTEGER,SAVE,POINTER:: IITER
        INTEGER,SAVE,POINTER:: IADAMPGMG
        INTEGER,SAVE,POINTER:: ISM
        INTEGER,SAVE,POINTER:: ISC
        INTEGER,SAVE,POINTER:: IOUTGMG
        INTEGER,SAVE,POINTER:: ISIZ
        INTEGER,SAVE,POINTER:: IPREC
        INTEGER,SAVE,POINTER:: IIOUT
        INTEGER,SAVE,POINTER:: SITER
        INTEGER,SAVE,POINTER:: TSITER
        TYPE ( C_PTR ),SAVE   ::GMGID
        INTEGER,SAVE,POINTER  ::IUNITMHC
        REAL,SAVE,POINTER:: HCLOSEGMG
        REAL,SAVE,POINTER:: RCLOSEGMG
        REAL,SAVE,POINTER:: DAMPGMG
        REAL,SAVE,POINTER:: DUP
        REAL,SAVE,POINTER:: DLOW
        REAL,SAVE,POINTER:: CHGLIMIT
        REAL   ,SAVE,POINTER,DIMENSION(:,:,:)::HNEWLAST
        DOUBLE PRECISION,SAVE,POINTER :: BIGHEADCHG
        DOUBLE PRECISION,SAVE,POINTER  :: RELAXGMG
      TYPE GMGTYPE
        INTEGER,POINTER:: IITER
        INTEGER,POINTER:: IADAMPGMG
        INTEGER,POINTER:: ISM
        INTEGER,POINTER:: ISC
        INTEGER,POINTER:: IOUTGMG
        INTEGER,POINTER:: ISIZ
        INTEGER,POINTER:: IPREC
        INTEGER,POINTER:: IIOUT
        INTEGER,POINTER:: SITER
        INTEGER,POINTER:: TSITER
        TYPE ( C_PTR )   ::GMGID
        INTEGER,POINTER  ::IUNITMHC
        REAL,POINTER:: HCLOSEGMG
        REAL,POINTER:: RCLOSEGMG
        REAL,POINTER:: DAMPGMG
        REAL,POINTER:: DUP
        REAL,POINTER:: DLOW
        REAL,POINTER:: CHGLIMIT
...
!--------------------------------------------------------------------
!     READS INPUT FROM FILE TYPE GMG SPECIFIED IN NAME FILE
!     ALLOCATES GMG SOLVER
!     EXPLICIT DECLARATIONS
!--------------------------------------------------------------------
      USE GLOBAL,   ONLY:IOUT,NCOL,NROW,NLAY
      USE GMGMODULE,ONLY:IITER,IADAMPGMG,ISM,ISC,IOUTGMG,ISIZ,&
     &                   IPREC,IIOUT,SITER,TSITER,GMGID,HCLOSEGMG,&
     &                   RCLOSEGMG,DAMPGMG,RELAXGMG,&
     &                   IUNITMHC,DUP,DLOW,CHGLIMIT,HNEWLAST,&
     &                   BIGHEADCHG
      USE GMG_C_INTERFACE
      IMPLICIT NONE
      CHARACTER*200 LINE
...
!--------------------------------------------------------------------
      ...
      END IF
!
!---- CHECK FOR FORCED DOUBLE PRECISION
!
      IPREC=0
...
!***********************************************************************
     ...
     &                  HNEWLAST)
!***********************************************************************
!     GMG7AP CALLS THE FOLLOWING FUNCTIONS FROM THE GMG LIBRARY:
!
!    MF2KMG_ASSEMBLE:
!      -- ASSEMBLES CELL-CENTERED FINITE DIFFERENCE
!         MATRIX AND MULTIGRID PRECONDITIONED CONJUGATE
!         GRADIENT SOLVER.  RETURNS L2-NORM OF RESIDUAL BIGR0
!
!    MF2KMG_EVAL:
!      -- APPROXIMATES THE HEAD CHANGE E FOR A*E=R WHERE A IS THE
!         CCFD MATRIX, AND R IS THE INITIAL RESIDUAL.
!         RETURNS THE L2-NORM OF THE RESIDUAL R-A*E.
!
!    MF2KGMG_BIGH:
!      -- COMPUTES MAX HEAD CHANGE BIGH AND RETURNS LOCATION
!         (COL,ROW,LAY) OF MAX HEAD CHANGE.  ABSOLUTE VALUE
!         OF BIGH IS MAX-NORM OF HEAD CHANGE.
!
!    MF2KMG_UPDATE:
!      -- ADDS THE CORRECTION TO THE HEADS HNEW=HNEW+DAMP*E.
!--------------------------------------------------------------------
      USE ISO_C_BINDING
      USE GMG_C_INTERFACE
      IMPLICIT NONE
...
!
      ...
      DOUBLE PRECISION BIGR0
      DOUBLE PRECISION S
      DOUBLE PRECISION DH
      DOUBLE PRECISION DAMP0
...
!
!--------------------------------------------------------------------
!  Save HNEW if MHC is being used.
!--------------------------------------------------------------------
...
!
!--------------------------------------------------------------------
!     SCALE CLOSURE CRITERION FOR INNER ITERATION BASED ON CURRENT
!     VALUE OF DAMPING AND INITIAL RESIDUAL.
!--------------------------------------------------------------------
...
!  Relative Reduced Residual Method
        RSQ=BIGR
        DAMPA=DAMP
        BIGHA=BIGH
...
!
!--------------------------------------------------------------------
!  Call MHC to Compute Head Change if MHC is active
!--------------------------------------------------------------------
      IF(IADAMP.EQ.2 .OR. IUNITMHC.GT.0)&
     &  CALL MHC7AP(IUNITMHC,KITER,KSTP,KPER,NCOL,NROW,NLAY,IBOUND,&
...
!***********************************************************************
!     SUBROUTINE RESPRINT IS CALLED FROM THE SOLVER FOR OUTPUT
!     OF REDUCTION HISTORY.
!
!     THE ITERATION (I), THE RESIDUAL (RES), AND THE
!     CONVERGENCE FACTOR (CFAC) ARE PRINTED.
!***********************************************************************
...
!
!-----MODIFIED FROM LMG (MEHL & HILL, 2001, USGS OFR 01-177), SUBROUTINE
!     ADAMP2 VERSION 1.1 22JUL2002.  MODIFICATIONS ARE DOCUMENTED IN
!     BANTA, 2006, IGWMC, "MODFLOW AND MORE, 2006" CONFERENCE PROCEEDINGS,
!     P. 596-600.
!     ******************************************************************
!     THIS SUBROUTINE CALCULATES THE DAMPING PARAMETER USING THE
!     RESIDUAL REDUCTION METHOD, AS MODIFIED BY BANTA, 2006.
!     ******************************************************************
!        SPECIFICATIONS:
!     ------------------------------------------------------------------
      IMPLICIT NONE
!     !   Argument list variables
      ...
      REAL,INTENT(IN):: BIGHPCG
      REAL,INTENT(IN):: CHGLIMIT
      REAL,INTENT(IN):: DLOW
      REAL,INTENT(IN):: DUP
      DOUBLE PRECISION, INTENT(IN)    :: BIGHEADCHG
      REAL,             INTENT(INOUT) :: DAMP
      DOUBLE PRECISION,INTENT(INOUT):: RSQ1
      DOUBLE PRECISION,INTENT(INOUT):: DDAMP
!     !   Local variables
      LOGICAL:: OSCIL
      LOGICAL:: OSCILH
      LOGICAL:: OSCILLATING
      DOUBLE PRECISION:: RSQ2
      DOUBLE PRECISION:: RSQ3
      DOUBLE PRECISION:: DDLOW
      DOUBLE PRECISION:: DDUP
      DOUBLE PRECISION:: BIGH1
      DOUBLE PRECISION:: BIGH2
      DOUBLE PRECISION:: BIGH3
      DOUBLE PRECISION:: BIGHABS
      DOUBLE PRECISION:: DDAMPTMP
      DOUBLE PRECISION:: DDLOWINTRL
      DOUBLE PRECISION:: DDUPINTRL
      DOUBLE PRECISION:: HCINC
      DOUBLE PRECISION:: HCDEC
      INTEGER:: IPER
      INTEGER:: IMULT
      INTEGER:: ICONST
      INTEGER:: NRAN
...
!1
!1------CALCULATE RESIDUAL AND INITIALIZE VARIABLES FOR FIRST ITERATION
      RSQ1=DSQRT(RSQ1)
      OSCIL = .FALSE.
      OSCILH = .FALSE.
      IF(KITER .EQ. 1)THEN
        ...
        KSMALLERDAMP=0
      ELSE
!2------CALCULATE THE RELATIVE REDUCTION IN THE RESIDUAL FOR THIS
!2------ITERATION ("RRED") SCALED BY THE DAMPING.
!2A-----IF THE REDUCTION IS GREATER THAN 50%, THEN USE THAT RELATIVE
!2A-----REDUCTION AS THE DAMPING VALUE FOR THE NEXT ITERATION.  THE IDEA
!2A-----HERE IS WHEN WE ARE IN A LINEAR PART OF THE SOLUTION, THE "RRED"
!2A-----WILL MOVE TOWARDS VALUES NEAR OR > 1, AND LITTLE OR NO DAMPING
!2A-----WILL BE APPLIED, WHICH IS APPROPRIATE.
        BIGH3=BIGH2
        BIGH2=BIGH1
        BIGH1=BIGHEADCHG
        ...
        IF(RRED .GT. 0.5)THEN
!          DDAMP=RRED
! check if we've had 2 successive lowerings of the residuals
          IF(RSQ3 .GT. RSQ2 .AND. RSQ2 .GT. RSQ1)THEN
            DDAMP=0.5*(DDAMP + MIN(DUP,RRED)) ! increase DAMP slower
            IF(DDAMP .LT. DLOW) DDAMP=DLOW
! check if we were doing better before.  If so, use previous damping
...
!         !  Solution is diverging--set damp to DLOW
          DDAMP=DLOW
        ELSE
!2B-----THE RELATIVE REDUCTION IS LESS THAN 50%, INDICATING THE
!2B-----SOLUTION IS NOT PROGRESSING ADEQUATELY.  BASED ON HOW FAR FROM
!2B-----"RRED" IS FROM 50%, THE DAMPING IS MORE OR LESS AGGRESSIVE.
!2B-----NOTE: FOR THIS CASE, THE MAXIMUM VALUE OF DAMP WILL BE 0.3 + DLOW
!2B-----SO SETTING DLOW=0.2 IS APPROPRIATE SO DAMP WON'T EXCEED 0.5
          DDAMP=0.075/(0.75-RRED) + DLOW
        ENDIF
!3
!3-----CHECK TO SEE IF SOLUTION IS OSCILLATING BY CHECKING THE
!3-----RELATIVE DIFFERENCE BETWEEN THE CURRENT RESIDUAL AND THE
!3-----RESIDUAL FROM TWO ITERATIONS AGO.  IF THEY ARE W/ IN 10%, THEN
!3-----THE SOLUTION IS OSCILLATING.
!3-----MAKE SURE DDAMP DOESN'T EXCEED DUP
!3-----IF THE SOLUTION IS OSCILLATING, AND WE ARE APPLYING A SIMILAR
!3-----VALUE OF DAMP (W/ IN 1%) THAT WE APPLIED PREVIOUSLY, THEN WE ARE
!3-----CAUGHT IN SOME TYPE OF ATTRACTOR. SO GENERATE A RANDOM VALUE OF
!3-----DDAMP AND HOPE TO GET OUT OF THE BASIN OF THIS ATTRACTOR.
!3-----NOTE THAT THIS IS VERY MUCH A DESPERATION EFFORT!
!
...
!4
!4-----STORE VALUES OF DAMP AND RESIDUALS FOR NEXT ITERATION
        RSQ3=RSQ2
        DAMP3=DAMP
      ENDIF
      RSQ2=RSQ1
...
!  Set pointers to GMG data for a grid
      USE GMGMODULE
!
      IMPLICIT NONE
      INTEGER :: IGRID
