...
!
      ...
      ENDIF
!
!     Test for creation of a WEL1 package and auxiliary output files
!
      iok = 1
      DO WHILE ( iok.EQ.1 )
        CALL NCREAD(In, txt, ierr)
...
!            Find and use file name
     ...
     &          WRITE (ABS(IOWELL2(jf)),'(3i10)') MXWEL2, IWL2CB, IWELPT
      ...
      ENDDO
!
!  Write header in Auxiliary BYNODE file if KPER=1 & IO>0
!
      ...
      ENDIF
!
!  Write header in Auxiliary QSUM file if KPER=1 & IO>0
!
...
!
      ...
      ENDIF
!
!  Write blank fields in Auxiliary BYNODE file if KPER=1 & IO>0
!
      ...
      ENDIF
!
!  Write blank fields in Auxiliary QSUM file if KPER=1 & IO>0
!
...
! -----print the individual rates if requested(IWL2CB<0).
     ...
     &               HNEW(i,j,k), dd, qwbar, sL, sNL
            ENDIF
!
! -----print the individual rates to auxiliary file if requested(IWL2CB<0).
...
! -----print the summed rates if requested(IWL2CB<0).
     ...
     &                              qwbar
! -----print the summed rates to auxiliary file if requested .
...
!------------------------------------------------------------------
!
!   Test auxiliary output files for cleaning data sets
      iobynd = ABS(IOWELL2(2))
      IF ( iobynd.GT.0 ) THEN
...
!
      ...
      CALL UPCASE(tx2)
!
!    Test for switching control to an auxiliary input file
!
      ki = INDEX(Txt, ':')
      ...
      ENDIF
!
!    Test for returning io control from auxiliary input to master input file
!
...
!
      ...
      RETURN
!
!    Report error in opening auxiliary input file and stop
!
20    WRITE (*, 25) afile
