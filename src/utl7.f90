...
!5------CHECK FOR AUXILIARY VARIABLE "AUXSFAC" AND STORE LOCATION 
      DO JJ=1,NAUX
        ...
        END IF
      END DO
!5A-----READ THE LIST -- BINARY OR ASCII
      IF (IBINARY.NE.0) THEN
        READ(IN) ((RLIST(JJ,II),JJ=1,NREAD2),II=LSTBEG,N)
!5B-----READ AN ASCII LIST
      ELSE
!          
        DO 240 II=LSTBEG,N
!
!5C-----Read a line into the buffer.  (The first line has already been
!5C-----read to scan for EXTERNAL and SFAC records.)
        IF(II.NE.LSTBEG) READ(IN,'(A)') LINE
!
!5D-----Get the non-optional values from the line.
        IF(IFREFM.EQ.0) THEN
