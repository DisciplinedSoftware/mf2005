...
!16-----ARE CONSISTENT WITH STARTING HEADS.
...
! block IF construct immediately below and placed here so that it would
...
110    CONTINUE
      ...
      IF(NDF) THEN
       ...
       END IF
!
!5------INITIALIZE VARIABLES FOR DIRECT SOLUTION OF HEAD IN INTERBED
       ...
       IITER = 0
       DHMAX = ZERO
!-------ITERATE DIRECT SOLUTION OF HEAD IN INTERBED. ITERATION IS
!-------PERFORMED TO ENSURE STORAGE COEFFICIENT FOR THE CURRENT
!-------OUTER ITERATION IS CONSISTENT WITH THE INTERBED HEAD.       
       DBITER: DO
         IITER = IITER + 1
!
!-------ASSEMBLE COEFFICIENTS FOR DIRECT SOLUTION OF HEAD IN INTERBED
         IF (IDBIT.LT.1) THEN
...
!6--------ADD CHANGES TO HEAD VALUES TO GET HEAD AT CURRENT ITERATION.
         ...
         END DO
!
!---------TERMINATE IF DELAY BED ITERATION NOT ALLOWED
!         THIS IF THE ORIGINAL SUB IMPLEMENTATION
         IF (IDBIT.LT.1) EXIT DBITER
!
!---------TERMINATE IF AC2 IS NOT ONE
         IF (AC2.NE.ONE) EXIT DBITER
!
!---------TERMINATE IF DHMAX IS LESS THAN SMALL CRITERIA
...
!
!-------ORIGINAL APPROACH - LOADED ON RHS
      IF (IDBIT.LT.1) THEN
        STRGS=ZERO
        L1=LOC3
        L2=LOC3+NN-1
...
!
!-------REVISED APPROACH - LOADED ON HCOF AND RHS
      ELSE
!
!--------CALCULATE EXCHANGE OF GROUNDWATER WITH DELAY INTERBEDS
        HHNEW=DH(LOC3)
