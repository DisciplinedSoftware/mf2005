!     Last change:  ERB  23 Aug 2001    9:26 am
!C
       PROGRAM HYDFMT
!C
!----- Reads unformatted data from the HYDMOD package and writes out
!----- columns as formatted data to selected files by topic. The files
!----- have extensions that represent the type of data they contain.
!----- The three-letter extensions are as follows:
!-----      TYPE OF DATA                                      OUTPUT FILE
!-----                                                          EXTENSION
!----- (1)  ground-water levels (head)                              .gwh
!----- (2)  ground-water level decline (drawdown)                   .gwd
!----- (3)  ground-water preconsolidation level (critical head)     .crh
!----- (4)  layer-specific compaction (compaction)                  .cmp
!----- (5)  sum of all layer compaction (subsidence)                .sub
!----- (6)  surface-water streamflow water level (stage)            .sth
!----- (7)  surface-water streamflow into a reach (inflow)          .inf
!----- (8)  surface-water streamflow out of a reach (inflow)        .otf
!----- (9)  flow between ground-water & surface-water (leakage)     .gsf
!-----
!----- R.T. Hanson, USGS-WRD, San Diego, California, rthanson@usgs.gov
!----- S.A. Leake, USGS-WRD, Tucson, Arizona, saleake@usgs.gov
!-----
...
!     ! (Fortran 2003 standard, equivalent to Intel's FORM='BINARY')
     ...
     &        STATUS='OLD')
!C
!----- Read first unformatted header record with number of hydrographs saved.
!----- Negative number indicates double precision
      ...
      END IF
!----- Read second unformatted header record with labels of hydrographs saved
...
!c
!----- Read the number of columns to be written to output file
!----- The number of columns does not include the simulation time which is
!----- always the first column in the formatted file.
...
5      WRITE(*,*) 'Enter number of columns to be read:'
       ...
       WRITE(*,10) NC
       GEN=.FALSE.
!----- Read the column numbers, free format, any order
10     FORMAT('Enter ',I3,' column numbers:')
       READ(*,*) (JCOL(N),N=1,NC)
!----- Read format for output of data, free format, no quotes.  Example:
!-----      (F10.2,4F10.3)
!----- This example writes simulation time with F10.3 and thirty columns of
!----- observations with F10.2
...
20     FORMAT(' Column number',I3,' is greater than maximum possible of'&
     &         ,I3,/,' please reenter column numbers...')
       GO TO 5
      ENDIF
25    CONTINUE
      ENDIF
!----- Read the unformatted and Write the formatted results
30    IF(IPREC.EQ.1) THEN
!        READ(inunit,ERR=50) TIME,(Z(N),N=1,NUMH) ! Use when access=transparent
        READ(inunit,END=50) TIME,(Z(N),N=1,NUMH)
      ELSE
!        READ(inunit,ERR=50) DTIME,(DZ(N),N=1,NUMH) ! Use when access=transparent
...
31      CONTINUE
      END IF
!----- Convert model elapsed time into calendar time
...
!C
!-----
!----- VERSION 0100 29JULY1998 MODTIME
!     ***********************************************************************
!     CONVERT ELAPSED MODEL TIME TO DECIMAL OR CALENDAR DATES FOR HYDROGRAPHS
!     ***********************************************************************
!
!     SPECIFICATIONS:
!     ------------------------------------------------------------------
...
!C
     ...
     &'BASIC PACKAGE DATA-->ITMUNI'
      ...
      ENDIF
!C
!C    SET MULTIPLIER BASED ON MODEL TIME, FOR DATE FORMAT CONVERSION
!C        TO ELAPSED DAYS
!C
      IF(TIMTYP.EQ.'C'.or.TIMTYP.EQ.'c')THEN
       IF(ITMUNI.EQ.1)THEN
     ...
     &       (MOD(YY,400.).EQ.0.))THEN
       ...
       ENDIF
!C
!C    SET MULTIPLIER BASED ON MODEL TIME FOR DECIMAL TIME FLAG ELPTYP
!C
      ELSE IF(TIMTYP.EQ.'D'.or.TIMTYP.EQ.'d')THEN
...
!-----
!----- VERSION 0100 29JULY1998 DAYS
!     ***********************************************************************
!C     FOR DATE GIVEN AS AN INTEGER YYMMDD, THIS FUNCTION RETURNS
!C     THE NUMBER OF DAYS SINCE DECEMBER 31, 1899.
!     ***********************************************************************
!
!     SPECIFICATIONS:
!     ------------------------------------------------------------------
...
!-----
!----- VERSION 0100 29JULY1998 DATE1
!     ***********************************************************************
!C     FOR VALUE OF DAYS SINCE DECEMBER 31, 1899 FUNCTION RETURNS THE
!C     DATE AS AN INTEGER IN THE FORM YYMMDD
!     ***********************************************************************
!
!     SPECIFICATIONS:
!     ------------------------------------------------------------------
...
!-----
!----- VERSION 0100 29JULY1998 LNOTE
!     ***********************************************************************
!C     RETURNS THE LEFT-JUSTIFIED, NONBLANK LENGTH
...
!-----
!----- VERSION 0100 29JULY1998 LEFTJ
!     ***********************************************************************
!C     RETURNS THE LEFT-JUSTIFIED, NONBLANK
!C     PORTION OF A CHARACTER STRING
!     ***********************************************************************
!
!     SPECIFICATIONS:
!     ------------------------------------------------------------------
