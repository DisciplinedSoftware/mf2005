!     ******************************************************************
!     MAIN CODE FOR U.S. GEOLOGICAL SURVEY MODULAR MODEL -- MODFLOW-2005
!     ******************************************************************
!
!        SPECIFICATIONS:
!     ------------------------------------------------------------------
...
!-------ASSIGN VERSION NUMBER AND DATE
      ...
      CHARACTER*10 MFVNAM
      PARAMETER (VERSION='1.12.02 2/11/2026')
...
!----------READ USING PACKAGE READ AND PREPARE MODULES.
     ...
     &                                     IUNIT(22),KKPER,KKSTP,NSOL,&
     &                                     IOUTS,IUNIT(1),IUNIT(23),&
     &                                     IUNIT(37),IUNIT(55),IGRID)
        IF(IUNIT(43).GT.0 .AND. IUNIT(44).GT.0)&
...
!7C2A---FORMULATE THE FINITE DIFFERENCE EQUATIONS.
     ...
     &                              IUNIT(22),IUNIT(8),IUNIT(55),IGRID)
...
!             BECAUSE SWI2 SAVES THE RHS (RHSPRESWI) PRIOR TO ADDING SWI TERMS
...
159         CONTINUE
          ENDIF
!
!-------SAVE SWI BUDGET INFORMATION BEFORE REST OF BOUNDARY PACKAGES
!-------SINCE IT HAS CORRECTIONS TO FACE FLOWS 
...
!     Write times to file if requested
      ...
      ENDIF
!
!     Ensure output file is written on macOS/gfortran, but flush only IOUT to avoid
!     access violations with stream-access binary files using ifx on linux/windows.
      CALL FLUSH(IOUT)
!
      RETURN
      END
