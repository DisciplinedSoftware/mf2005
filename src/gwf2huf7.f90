...
!4------CALCULATE HOR. CONDUCTANCE(CR AND CC) FOR CONSTANT T LAYERS.
      DO 70 K=1,NLAY
!-------CONVERT HK TO HYDRAULIC CONDUCTIVITY AND SC1 TO SPECIFIC STORAGE
        DO 71 I=1,NROW
        DO 71 J=1,NCOL
...
!3------LOOP THROUGH EACH CELL, AND CALCULATE SATURATED THICKNESS.
      ...
      DO 200 J=1,NCOL
!
!3A-----SET SATURATED THICKNESS=0. FOR DRY CELLS.
...
!-----------------Old head is below and new head is above this unit
                    CRHS=CRHS+RMLT0*BNP*(BOTU-TOPU)
                  ENDIF
                ENDIF
              ENDIF
...
!
     ...
     &                        HNWHGU(J,I,NU)-QN*0.5*THCKU/VK
            ENDIF
          ELSE
!---------FIRST, PROPAGATE HEAD FROM MIDDLE OF CELL TO MIDDLE OF UNIT
            DO 400 KL=KT,KB
            ...
            ENDIF
400         CONTINUE
!---------NOW, PROPAGATE CELL TO BOTTOM OF UNIT
